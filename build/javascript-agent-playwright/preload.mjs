...
async function resolveRerunTestList(runContextRaw) {
  ...
  try {
    ...
    // Each Zebrunner test name looks like
    // "<project> > <project> > <relative/file/path.spec.ts> > <title...>".
    // Playwright's --test-list format is "[project] › file › title...",
    // matching by exact file path + title-path prefix (no regex, no
    // escaping) — so we only need to drop the duplicated leading project
    // segment and re-join the rest.
    const lines = new Set();
    for (const test of exchanged.testsToRun ?? []) {
      const segments = test.name.split(" > ").map((segment) => segment.trim());
      if (segments.length < 3) {
        continue;
      }
      const [project, , ...rest] = segments;
      lines.add(`[${project}] › ${rest.join(" › ")}`);
    }
    if (lines.size === 0) {
